...
/**
 * @brief ChaCha20 pseudo-random number generator (CSPRNG) implementation.
 * 
 * ChaCha20 is a stream cipher designed by Daniel J. Bernstein. It operates on 256-bit keys
 * and uses a 64-bit counter. It is known for its high speed and security, but it is not designed
 * for encryption purposes. Its sole purpose is to provide cryptographically secure random numbers.
 * 
 * @details
 * ChaCha20 is a fast, secure algorithm that is widely used for cryptographic applications,
 * such as generating random numbers in a way that is resistant to certain types of attacks.
 * It is based on the ChaCha stream cipher, and it provides high throughput, making it a 
 * good choice when performance is a key factor. However, it is important to note that this
 * implementation is specifically intended for random number generation, not encryption.
 * 
 * @warning Ensure that the random device (`std::random_device`) used as the seed
 * is of high entropy and is properly initialized.
 * 
 * @note Best used when you need a high level of security, especially in cryptographic contexts
 * where unpredictability and resistance to attacks are paramount. However, this implementation
...
/**
 * @brief AES-CTR (Counter mode) pseudo-random number generator (CSPRNG) implementation.
 * 
 * AES-CTR is based on the AES encryption algorithm in counter mode. It generates pseudo-random
 * numbers by encrypting a counter with a secret key, but it is not designed for encryption
 * purposes. Its sole purpose is to provide cryptographically secure random numbers.
 * 
 * @details
 * AES-CTR is a strong CSPRNG as it uses the AES encryption algorithm, which is well-tested
 * and considered secure. The random numbers are generated by encrypting a nonce and a counter,
 * then using the output as a random stream. The key expansion ensures that the key is 
 * securely applied to each encryption round.
 * 
 * @warning AES-CTR requires a secure key to avoid vulnerabilities. The key must not be reused
 * with the same nonce for different applications.
 * 
 * @note AES-CTR is a good choice when you need high-quality randomness and performance
 * while leveraging AES's widespread cryptographic trust. However, it is important to note that
 * this implementation is specifically intended for random number generation, not encryption.
 * 
